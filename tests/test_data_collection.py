"""Tests for LegiScan API data collection functionality."""
import json
import tempfile
...
def test_get_session_list(mock_session_list_response):
    ...
    with patch('src.data_collection.fetch_api_data', return_value=mock_session_list_response):
...
def test_get_session_list_empty():
    ...
    with patch('src.data_collection.fetch_api_data', return_value={"status": "OK", "sessions": []}):
        sessions = get_session_list('ID', [2023, 2024])
        assert len(sessions) == 0

def test_get_session_list_error():
    """Test handling of API error."""
    with patch('src.data_collection.fetch_api_data', return_value=None):
        sessions = get_session_list('ID', [2023, 2024])
        assert len(sessions) == 0
